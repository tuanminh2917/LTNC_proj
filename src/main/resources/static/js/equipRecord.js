...
// Save: chỉ kiểm tra và gửi các dòng mới vừa thêm.
async function handleSaveButtonClick() {
    ...
    if (!equipNameField.textContent.trim() || !receiveDateField.textContent.trim()) {
        alert("Vui lòng nhập mã thiết bị hợp lệ trước khi lưu.");
        return;
    }

    const newRows = tbody.querySelectorAll("tr.new-record-row");

    if (newRows.length === 0) {
        alert("Chưa có hồ sơ mới để lưu.");
        return;
    }
    ...
    for (const row of newRows) {
        ...
        if (!scopeOfWork) {
            alert("Nội dung bảo trì, sửa chữa không được để trống.");
            return;
        }
    ...
    try {
        const response = await fetch(`/api/records/add/${equipId}`, {
            method: "POST",
            headers: {
        ...
        alert("Đã xảy ra lỗi khi lưu hồ sơ trang thiết bị.");
    }
}