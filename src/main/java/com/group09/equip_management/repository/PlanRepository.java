...
public interface PlanRepository extends JpaRepository<Plan, Integer> {
    ...
    @Query(value = "SELECT DISTINCT p.* FROM Plan p " +
                   ...
                   "(:createdDate IS NULL OR p.created_date = :createdDate) AND " +
                   "(:equipId IS NULL OR :equipId = '' OR pd.equip_id LIKE CONCAT('%', :equipId, '%')) AND " +
                   "(:conductor IS NULL OR :conductor = '' OR pd.conductor LIKE CONCAT('%', :conductor, '%')) AND " +
                   "(:scopeOfWork IS NULL OR :scopeOfWork = '' OR pd.scope_of_work LIKE CONCAT('%', :scopeOfWork, '%'))", // <-- BỔ SUNG TẠI ĐÂY (Đã bỏ chữ AND thừa ở cuối)
           nativeQuery = true)
    List<Plan> searchPlans(@Param("type") String type,
                           ...
                           @Param("scopeOfWork") String scopeOfWork); // <-- THÊM PARAM MỚI Ở ĐÂY
